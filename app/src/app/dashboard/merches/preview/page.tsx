  ...
  return (
    ...
    >
      ...
      <div className={`space-y-2 px-4 py-4 ${compact ? "px-3 py-3" : ""}`}>
        ...
        <p
          className={`font-medium uppercase tracking-wide text-[#A3A3A3] ${compact ? "text-[10px]" : "text-xs"}`}
        >
          {item.detail}
        </p>
        <div
          className={`flex items-center gap-2 text-[#C9C9C9] ${compact ? "text-[10px]" : "text-xs"}`}
        >
          <span>{item.date}</span>
          <span>{item.time}</span>
        </div>
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-[#A3A3A3]">
            See how your merch appears to fans before publishing.
          </p>
        </div>
      ...
      >
        <div className="mb-6 flex items-center justify-between border-b border-[#1F1F1F] pb-4">
          ...
          {!isMobile && <p className="text-xs text-[#A3A3A3]">Fan-facing view — read only</p>}
        </div>
