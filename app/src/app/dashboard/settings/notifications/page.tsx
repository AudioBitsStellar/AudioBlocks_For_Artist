...
export default function NotificationPreferencesPage() {
  ...
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(
        'audioblocks:notification-preferences:v1',
      );
      if (raw) {
  ...
  const isDirty = useMemo(
    () => !prefsEqual(prefs, DEFAULT_NOTIFICATION_PREFERENCES),
    [prefs],
  );
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const allDisabled = (
      Object.keys(prefs) as NotificationEventKey[]
    ).every((key) => !prefs[key].email && !prefs[key].inApp);
    if (allDisabled) {
      firstToggleRef.current?.focus();
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Choose how you want to hear from us for each kind of event. Changes are
            saved locally so you don't lose them on this device.
          </p>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-border-subtle text-xs uppercase tracking-wide text-text-muted">
              <tr>
                ...
                <th scope="col" className="px-6 py-4 text-center">In-app</th>
              </tr>
            </thead>
            <tbody>
