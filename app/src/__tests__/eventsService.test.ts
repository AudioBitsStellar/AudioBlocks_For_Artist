...
describe("useEventsService — CRUD operations", () => {
  ...
  describe("useCreateEvent", () => {
    it("creates an event and returns it", async () => {
      ...
      const { result } = renderHook(() => useEventsService().useCreateEvent(), {
        wrapper: Wrapper,
      });
  ...
  describe("useUpdateEvent", () => {
    it("updates an event with a partial payload", async () => {
      ...
      const { result } = renderHook(() => useEventsService().useUpdateEvent(1), {
        wrapper: Wrapper,
      });
    ...
    it("rejects when the update API fails", async () => {
      ...
      const { result } = renderHook(() => useEventsService().useUpdateEvent(99), {
        wrapper: Wrapper,
      });

      await act(async () => {
        const partial: Partial<CreateEventPayload> = { title: "Nonexistent Event" };
        await expect(result.current.mutateAsync(partial)).rejects.toThrow(
  ...
  describe("useDeleteEvent", () => {
    it("deletes an event successfully", async () => {
      ...
      const { result } = renderHook(() => useEventsService().useDeleteEvent(1), {
        wrapper: Wrapper,
      });
    ...
    it("rejects when the delete API fails", async () => {
      ...
      const { result } = renderHook(() => useEventsService().useDeleteEvent(999), {
        wrapper: Wrapper,
      });
