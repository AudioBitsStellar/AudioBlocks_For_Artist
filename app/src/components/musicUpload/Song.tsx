...
const Song = () => {
  ...
  const handleRetry = async () => {
    ...
    if (retryCount >= MAX_RETRY_ATTEMPTS) {
      setUploadedFile((prev) => (prev ? { ...prev, status: "failed" } : prev));
      toast.error(
        `Maximum retry attempts (${MAX_RETRY_ATTEMPTS}) reached. Please delete and start over.`
      );
      return;
    }
    ...
    try {
      ...
      setUploadedFile((prev) => (prev ? { ...prev, status: "uploading" } : prev));

      await uploadSongInChunks(audioFile, fileId, failedChunkIndex);

      setUploadedFile((prev) => (prev ? { ...prev, status: "success" } : prev));
    } catch (err) {
      if (err instanceof Error && err.message === "UPLOAD_CANCELLED") {
        setUploadedFile((prev) => (prev ? { ...prev, status: "cancelled" } : prev));
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="space-y-5 col-span-2">
        <div className="space-y-2">
          ...
          />
          {errors.title && (
            <p id="song-title-error" className="text-[10px] text-red-500" role="alert">
              {errors.title.message}
            </p>
          )}
          {(watchedValues.title?.length ?? 0) >= 90 && (
            <p
              className={`text-[10px] text-right ${(watchedValues.title?.length ?? 0) >= 100 ? "text-red-500" : "text-yellow-500"}`}
            >
        ...
        <div className="space-y-2">
          ...
          />
          {errors.description && (
            <p id="song-description-error" className="text-[10px] text-red-500" role="alert">
              {errors.description.message}
            </p>
          )}
          {(watchedValues.description?.length ?? 0) >= 450 && (
            <p
              className={`text-[10px] text-right ${(watchedValues.description?.length ?? 0) >= 500 ? "text-red-500" : "text-yellow-500"}`}
            >
        ...
        <div className="space-y-2">
          ...
          {errors.genre && (
            <p id="song-genre-error" className="text-[10px] text-red-500" role="alert">
        ...
        <div className="space-y-2">
          ...
          />
          {errors.composer && (
            <p id="song-composer-error" className="text-[10px] text-red-500" role="alert">
              {errors.composer.message}
            </p>
          )}
          {(watchedValues.composer?.length ?? 0) >= 90 && (
            <p
              className={`text-[10px] text-right ${(watchedValues.composer?.length ?? 0) >= 100 ? "text-red-500" : "text-yellow-500"}`}
            >
      ...
      <div className="space-y-6  col-span-1">
        ...
        <div
          className="rounded-2xl border border-[#2A2A2A] bg-[#161616] p-6 w-full flex flex-col overflow-hidden"
          style={{ height: "192.33px" }}
        >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2">
                      <p className="text-[10px] text-[#A3A3A3]">
                        {uploadedFile.size} &middot; {uploadedFile.type}
                      </p>
